...
uint8 HalLedSet (uint8 leds, uint8 mode)
{
  ...
  switch (mode)
  {
    case HAL_LED_MODE_BLINK:
      ...
      HalLedBlink (leds, 1, HAL_LED_DEFAULT_DUTY_CYCLE, HAL_LED_DEFAULT_FLASH_TIME);//只闪烁一次
      break;

    case HAL_LED_MODE_FLASH:
      /* Default flash, N times, D% duty cycle */
      HalLedBlink (leds, HAL_LED_DEFAULT_FLASH_COUNT, HAL_LED_DEFAULT_DUTY_CYCLE, HAL_LED_DEFAULT_FLASH_TIME);//闪烁HAL_LED_DEFAULT_FLASH_COUNT次
      break;
...
void HalLedUpdate (void)
{
  ...
  led  = HAL_LED_1;
  leds = HAL_LED_ALL;
  sts = HalLedStatusControl.HalLedControlTable;//LED表

  /* Check if sleep is active or not */
  if (!HalLedStatusControl.sleepActive) //如果睡眠为真，则不执行该if
  {
    while (leds)
    {
      if (leds & led)
      {
        if (sts->mode & HAL_LED_MODE_BLINK)//对应该LED的模式为BLINK(闪烁)
        {
          time = osal_GetSystemClock(); //获得系统时钟
          if (time >= sts->next)//如果当前的时间比上次的存的NEXT的时间晚，就执行
          {
            if (sts->mode & HAL_LED_MODE_ON)//对应该LED的模式为 开
            {
              pct = 100 - sts->onPct;               /* Percentage of cycle for off */
              sts->mode &= ~HAL_LED_MODE_ON;        /* Say it's not on */
              HalLedOnOff (led, HAL_LED_MODE_OFF);  /* Turn it off */ //关掉该LED

              if ( !(sts->mode & HAL_LED_MODE_FLASH) )
              {
                sts->left--;                         // Not continuous, reduce count，每次都减少一次闪烁计数值
              }
            }
            else if ( !(sts->left) && !(sts->mode & HAL_LED_MODE_FLASH) )
            {
              sts->mode ^= HAL_LED_MODE_BLINK;       // No more blinks //如果已经计数减到0，或不再是闪烁模式，就不再闪烁
            }
            else                                     //如果还是闪烁模式，就执行打开该LED 
            {
              pct = sts->onPct;                      // Percentage of cycle for on
              sts->mode |= HAL_LED_MODE_ON;          // Say it's on
              HalLedOnOff( led, HAL_LED_MODE_ON );   // Turn it on
            }
            if (sts->mode & HAL_LED_MODE_BLINK)//如果还是闪烁模式
            {
              wait = (((uint32)pct * (uint32)sts->time) / 100);
              sts->next = time + wait;  //当前时间加上等待时间更新NEXT
            }
            else
            {
              /* no more blink, no more wait */   //不再是闪烁模式，就不再等待
              wait = 0;
              /* After blinking, set the LED back to the state before it blinks */
              HalLedSet (led, ((preBlinkState & led)!=0)?HAL_LED_MODE_ON:HAL_LED_MODE_OFF);//设置回原来的状态
              /* Clear the saved bit */
          ...
          {
            wait = sts->next - time;  /* Time left *///当前的时间和下一次的时间的绝对值
          }
        ...
        leds ^= led; //把当前的LED对应的LEDS的位置清零 
      }
      led <<= 1;//左移LED
      sts++; //偏移LED表的指针，让它指向下一个LED 
    }
...
void HalLedOnOff (uint8 leds, uint8 mode)
{
  ...
  if (mode)     
  {
    HalLedState |= leds;//如果还有保存当前的LED状态
  }
  else
  {
